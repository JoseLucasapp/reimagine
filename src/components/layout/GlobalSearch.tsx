...
import { globalSearch } from "@/data/dashboardData";
...
export function GlobalSearch() {
  ...
  useEffect(() => {
    if (query.trim().length < 1) setOpen(false);
  }, [query]);
  ...
  return (
    <div ref={ref} className="relative w-full max-w-md">
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs truncate" style={{ color: "var(--text-muted)" }}>{item.type} · {item.subtitle}</p>
                </div>
              </button>
            );
          })}
      ...
      {open && visibleResults.length === 0 && query.trim().length >= 1 && (
        <div
